...
import { HorizontalLayout, CenterLayout, Layout } from '../../layout'
...
export default class Pager extends Component {
    ...
    createItem = (index) => {
        return <Button className={cn('pager-item', { 'active': index === this.state.current })} handler={() => this.itemHandler(index)} clear={true} key={index}>{index}</Button>
    }
    ...
    render() {
        ...
        return <Layout className={cn(className, CLASS_NAME)}>
            ...
            <Button handler={this.firstHandler}>{'首页>'}</Button>
            {view}
            <Button handler={this.lastHandler}>{'<尾页'}}</Button>
